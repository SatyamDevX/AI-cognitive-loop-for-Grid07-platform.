...
Status: in progress. Config loading and secret-safe provider readiness checks are implemented first. Gemini has an explicit one-call smoke test with very small output tokens and thinking disabled; broader network-backed LLM calls remain manual-key gated.
